...
def normalize_pairs(value):
    ...
    Handles lists like [[r1,l1],[r2,l2]] or [[r1,r2,...],[l1,l2,...]].
    """
    if value is None:
        return []
    ...
    # Case 1: shape (2, N) => two rows: receptor indices and ligand indices
    if arr.ndim == 2 and arr.shape[0] == 2:
        pairs = []
        for i in range(arr.shape[1]):
            pairs.append((int(arr[0, i]), int(arr[1, i])))
        return pairs

    # Case 2: shape (N, 2) => explicit pair rows
    if arr.ndim == 2 and arr.shape[1] == 2:
        pairs = []
        for i in range(arr.shape[0]):
            pairs.append((int(arr[i, 0]), int(arr[i, 1])))
        return pairs
...
def build_feature_space(all_dicts):
    ...
    Feature format: INTERACTIONTYPE__recIDX__ligIDX
    """
    features = set()
...
def main():
    ...
    if args.make_tsne:
        ...
        perplexity = min(args.tsne_perplexity, max(5, X.shape[0] - 1))
        tsne = TSNE(
            n_components=2,
